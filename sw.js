// ============================================
// TripLite Service Worker v35
// v35: 住宿模块携程直达链接升级 - 全部改为 hotelId 单酒店页面直达
// v34: 强制清除旧缓存，修复用户端 data.js 缓存未更新问题
// v33: note.html hitlayer改用top/left/right/bottom，fitCanvas补全尺寸计算
// v32: note.html 新增 #hitlayer绕过平台注入的透明覆盖节点
// ============================================

var CACHE_NAME = 'triplite-v35';
    ...
    caches.open(CACHE_NAME).then(function(cache) {
      var tasks = PRECACHE_URLS.map(function(url) {
        return fetch(url, { cache: 'no-cache' })
          ...
          .catch(function(err) {
            // 单个资源失败不影响SW整体安装
            console.warn('[SW v35] precache skip:', url, err.message);
          });
      });
      return Promise.all(tasks);
    }).then(function() {
      console.log('[SW v35] install done');
    }).catch(function(err) {
      console.warn('[SW v35] install error (non-fatal):', err);
    })
  );
  // 强制立即激活，确保新SW立刻接管所有页面
  self.skipWaiting();
});
    ...
    caches.keys().then(function(keys) {
      return Promise.all(
        keys.filter(function(key) { return key !== CACHE_NAME; })
            .map(function(key) {
              console.log('[SW v35] delete old cache:', key);
